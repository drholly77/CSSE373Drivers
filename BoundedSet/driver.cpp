...
#include "wrapper.h"
#include "BoundedSet/BoundedSet1.hpp"
...
typedef BoundedSet1 <Text, maxSize, areEqual> TextSet;
#else
// Compiling in Debug configuration
#include "BoundedSet/BoundedSetChecking.hpp"
typedef BoundedSetChecking1 <Text, BoundedSet1 <Text, maxSize, areEqual>> TextSet;
#endif
...
)
{
	...
	index = getIndex();
	if (index == 1) {
		z = s1.remainingCapacity();
	}
	else {
		z = s2.remainingCapacity();
	}	// end if
	wcout << "s" << index << " remaining capacity = " << z << endl;
}	// doRemainingCapacity
